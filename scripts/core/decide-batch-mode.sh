...
if [ "$REVIEW_TYPE" = "存量审查" ] && [ "$ESTIMATED_TOKENS" -gt "$BATCH_TRIGGER_TOKENS" ]; then
  SIZE_BATCH_REQUIRED=true
fi

if [ "$PROJECT_TYPE" = "maven-multi" ] && [ "$REVIEW_TYPE" = "存量审查" ]; then
  if [ "$SIZE_BATCH_REQUIRED" = "true" ]; then
...
echo "BATCH_TRIGGER_TOKENS=$BATCH_TRIGGER_TOKENS"
