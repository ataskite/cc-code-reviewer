...
#   estimated_minutes = ceil(review_cost × mode_minutes / 260000)，下限 1
#
# 调用方式：
#   1) 作为 CLI：bash estimate-review-minutes.sh <REVIEW_MODE> <REVIEW_LINE_COUNT> <REVIEW_FILE_COUNT>
#      输出两行：
...
#   - 所有受支持模型统一按 1M 上下文估算，目标批次成本固定为 260000。
set -euo pipefail
...
# estimate_review_minutes <review_cost> <review_mode>
...
estimate_review_minutes() {
  ...
  local minutes
  minutes="$(ceil_div $((review_cost * target_minutes)) "$TARGET_REVIEW_COST")"
...
if [ "${BASH_SOURCE[0]:-$0}" = "$0" ]; then
  if [ "$#" -lt 3 ]; then
    echo "用法: bash estimate-review-minutes.sh <REVIEW_MODE> <REVIEW_LINE_COUNT> <REVIEW_FILE_COUNT>" >&2
  ...
  CLI_MINUTES="$(estimate_review_minutes "$CLI_COST" "$CLI_MODE")"
