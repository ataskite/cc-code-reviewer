...
> **跨文件风险信号（与文件类型无关）**：除文件类型表外，识别到以下任一信号时，无论文件被类型表归为哪一类，都必须将相关安全维度纳入评估，并按阶段 D 强制追踪取证。这些信号描述代码 shape 而非类名——安全风险跨命名、靠语义，仅按 `*Filter`/`*Security` 类名识别会漏掉名为 Service/Strategy/Handler 的鉴权链节点：
>   - **信号A 关卡/判定函数**：返回布尔/布尔语义值，语义为"放行·匹配·跳过·校验"（match/shouldFilter/allow/permit/canAccess/check/verify/validate/skip/preFilter）。任何返回 allow/true 或异常路径放行的分支需确认对应"已验证通过"。→ default-deny/fail-open（具体检查点见 `REVIEW_FRAMEWORK_PATH` 5.8 的 S1–S4）
>   - **信号B 归属/权限语义**：对资源按标识（id/code/sn）查询或修改，且涉及归属语义（owner/user/tenant/org/dept/merchant/creator 或任何区分数据主体的字段）。→ 对象级越权/多租户隔离（见 5.2）
>   - **信号C 外部输入到达 sink**：请求/参数/外部源数据流向查询/命令/文件路径/URL/反序列化/模板渲染。→ 注入/SSRF/路径穿越；可控性需跨文件追溯
>   - **信号D 对象经序列化/字符串化边界**：含敏感字段的对象进入日志/异常响应/序列化输出/`toString`。→ 间接信息泄露（见 5.3）
>   信号命中只激活检查、不改变定级门槛（仍按「安全问题分级边界」与 P0 五项硬门槛判定）。
...
4. **跨文件风险信号追踪（强制）**：当阶段 C 在某文件命中上方跨文件风险信号时：
   - **信号A/B 强制追踪**：无论该文件是否已产生正式问题，都必须跨文件取证。信号A（关卡函数）：查询该判定函数的**所有返回点与调用者**，确认 null/异常路径走向 allow 还是 deny（命中 5.8 S3 语义重载时查询字段写入点；命中 S4 编排无兜底时查询编排框架默认行为）。信号B（归属语义）：查询该资源查询/修改的**归属过滤条件**，确认是否绑定当前用户/租户。
   - **信号C/D 追溯后定级**：若单文件内无法证成，必须跨文件追溯（信号C：追溯外部输入数据流是否可达 sink；信号D：追溯对象是否被 logger/序列化消费）**后再定级，不得因"单文件不可证"直接降级或丢弃**（避免真实可控链被 P0 门槛压低后又被 fast 模式静默丢弃）。
   此追踪是信号能否成立为问题的关键证据来源，不得因"阶段 C 单文件内未发现正式问题"就跳过；追踪结果未达 P0/P1 门槛的，按正常分级处理（不得仅凭信号升级）。
