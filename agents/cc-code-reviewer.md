...
阶段 C 完成后，`REVIEW_MODE=security` 无条件执行第 5 项；其余项目仅在对应条件成立时进行**少量定向补充操作**：
...
5. **授权面二次扫描（仅 `REVIEW_MODE=security`，强制）**：全部正式文件完成首轮阅读后，按 `SECURITY_FRAMEWORK_PATH` 的“授权面二次扫描”重新遍历授权面台账，即使首轮没有越权候选也必须执行。对每行分别验证同角色异对象、低权限高功能、跨租户/组织、对象属性、批量/嵌套资源和替代执行路径六类差分反例；不适用必须有证据。`未绑定` 形成正式发现，`外部证据缺失` 形成待确认项；只有全部行已有结论且适用反例都有阻断证据或缺口时，才可写“未发现越权问题”。不得把登录态、不可预测 ID、注解/helper 名称、前端隐藏或“内部接口”标签当作授权成立证据。
...
**敏感值脱敏（强制）**：证据代码、配置片段与建议中不得原样复制密钥/口令/令牌/私钥等敏感值（password、secret、token、api key、连接串内嵌凭据等）——引用键名与位置、值以 `***` 掩码（如 `password: ***`），描述中说明值的形态（长度/字符集/是否 base64/hex）即可；发现本身照常输出（如「疑似硬编码密钥」）。报告会上传飞书等协作文档，原样搬运密钥等于二次泄露。
...
spring:
  datasource:
    ...
    password: ***        # ← 明文密码：证据已按脱敏契约掩码，只引用键名与位置，发现照常按硬编码凭据输出
```
...
- `REVIEW_MODE=security` 时必须输出 `## 授权面覆盖（仅 Security 模式）`，台账四类计数之和必须等于总行数；非 Security 模式省略
