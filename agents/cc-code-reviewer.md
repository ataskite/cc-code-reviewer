...
**前置条件**：第三步之后已将完整审查报告保存到本地 Markdown 文件。
...
并附上第三步生成的完整审查报告内容（包含所有章节：审查配置快照、执行摘要、各级别问题详情、修复优先级、总结等）。
...
在执行第三步（生成审查报告）时，请先读取该文件获取完整的格式要求。**特别注意其「问题条目通用规则」章节**：每个问题必须用三级标题 `### {编号} | [维度] 标题` 独立展开，包含完整的位置/证据/影响/建议字段；禁止把多个问题塌缩成一句话列表。
