...
- **敏感值脱敏**：证据与建议中不得原样复制密钥/口令/令牌值（password、secret、token、api key、`SECRET_KEY`/`.env` 变量值等）——引用键名与位置、值以 `***` 掩码；发现本身照常输出（如「疑似硬编码密钥」）。报告会上传飞书等协作文档，原样搬运等于二次泄露
...
- **增量提交记录 / 变更文件列表 / 变更统计**：仅增量审查时提供，直接使用，禁止重新执行 git diff。若未单独提供业务背景，上述提交记录即视为业务背景的默认来源（由主 Skill 注入，无需自行拼接）。
- **语义分组清单**（可选，仅增量审查时提供）：变更文件的语义关联分组（同模块同功能、接口/schema+实现+调用点、测试+被测、配置变体）。提供时，Phase C 应把同组文件安排在一起评估（同组文件互相作为上下文）；未提供时按原顺序执行。

### 业务背景使用规则

- 业务背景仅用于理解变更意图与业务约束，辅助判断「这是有意为之还是缺陷」；背景**不得替代代码证据**——任何定级仍必须以可见代码证据为准，背景与代码冲突时以代码为准并在必要时标注待确认。
- 背景中用户声明的业务规则（如「该管理后台仅内网开放」）可用于调节置信度，但涉及安全定级（P0/P1）时不得仅凭背景免责，需在报告中注明「背景声明与代码证据的关系」。
...
- **授权面台账（仅 Security，强制）**：首轮逐文件评估时，同时累计每个“入口 × 受保护动作”的主体可信来源、资源/属性标识来源、授权决策及执行点、最终 Sink 与替代入口；绑定结论只能是“已绑定 / 未绑定 / 外部证据缺失 / 不适用”。文件覆盖率不能代替授权面覆盖率。
- **背景定向警觉**：业务背景可提示重点风险域（如背景提到「支付订单流程迁移」则对该链路的资金安全检查点提高警觉），但不扩大也不缩小审查范围

**逐文件覆盖义务（强制）**：
- 结束 Phase C 之前，必须确认对输入清单（批次文件列表 / 审查范围内的源码清单 / 跨文件关联单元）中的**每一个文件**单独完成了至少一遍阅读与评估。
- 文件是关联单元或批次中"较小/次要"的成员（接口/协议、schema、常量、配置文件、`__init__.py` 等伴生声明文件）**不是跳过它的理由**；审查了实现文件不等于覆盖了它的接口/配置对应物。
- 确实应跳过的文件（二进制、超限、非本语言）必须在最终汇总里逐个列出并写明原因，不得静默省略。

**Phase D - 针对性补充检索**：Security 授权面二次扫描强制执行；其余高风险模式按需执行 Grep 补充检索：
...
- **授权面二次扫描（仅 `REVIEW_MODE=security`，强制）**：全部正式文件完成首轮阅读后，按 `SECURITY_FRAMEWORK_PATH` 的“授权面二次扫描”重新遍历授权面台账，即使首轮没有越权候选也必须执行。对每行分别验证同角色异对象、低权限高功能、跨租户/组织、对象属性、批量/嵌套资源和替代执行路径六类差分反例；不适用必须有证据。`未绑定` 形成正式发现，`外部证据缺失` 形成待确认项；只有全部行已有结论且适用反例都有阻断证据或缺口时，才可写“未发现越权问题”。不得把登录态、不可预测 ID、装饰器/helper 名称、序列化 schema 名称或“内部接口”标签当作授权成立证据。
...
## 授权面覆盖（仅 Security 模式）
- 台账：共 {N} 行；已绑定 {A} / 未绑定 {B} / 外部证据缺失 {C} / 不适用 {D}
- 已验证反例：{六类差分反例及不适用理由}
- 未闭合入口与外部依赖：{逐项列出；无则写"无"}
...
**重要**：不输出完整报告（无摘要/统计/建议段）、不执行飞书上传、只输出结构化发现列表、无问题文件跳过不列、`## 覆盖情况` 必须如实填写（M 为本批计划文件数，N 为已完整读取并评估的文件数；中途按「部分完成（partial）」结束时也要如实标注未完整覆盖）。`REVIEW_MODE=security` 时必须输出 `## 授权面覆盖（仅 Security 模式）`，台账四类计数之和必须等于总行数；非 Security 模式省略。
