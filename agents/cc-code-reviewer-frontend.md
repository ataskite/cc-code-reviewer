...
- **跨文件风险信号（与文件类型无关）**：除文件类型表外，识别到以下任一信号时，无论文件归为哪类，都必须将相关安全维度纳入评估，并按阶段 D 强制追踪取证。这些信号描述代码 shape 而非文件名——风险跨命名、靠语义：
  - **信号A 关卡/判定函数**：路由守卫/权限判定函数/中间件返回布尔，语义为"放行·匹配·跳过·校验"。任何返回放行/true 或异常路径放行的分支需确认对应"已验证通过"。→ 鉴权 fail-open（见维度 6）
  - **信号B 归属/权限语义**：BFF/Node 接口对资源按标识查询或修改，涉及归属语义（owner/user/tenant/org 或任何区分数据主体的字段）。→ 对象级越权/多租户隔离
  - **信号C 外部输入到达 sink**：URL 参数/外部输入流向 HTML 渲染/URL 拼接/跳转/BFF 下游请求。→ XSS/开放重定向/SSRF；可控性需跨文件追溯
  - **信号D 敏感字段进日志/埋点**：含 token/PII 的对象进入日志、错误上报（Sentry）、埋点。→ 间接信息泄露（见维度 9）
...
**跨文件风险信号追踪（强制）**：命中上方信号A/B 时强制跨文件追踪（无论是否已发现正式问题）；命中信号C/D 时，若单文件内无法证成则必须跨文件追溯后再定级，**不得因"单文件不可证"直接降级或丢弃**。
