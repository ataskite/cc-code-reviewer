...
- **敏感值脱敏**：证据与建议中不得原样复制密钥/口令/令牌值（password、secret、token、api key、`.env` 变量值等）——引用键名与位置、值以 `***` 掩码；发现本身照常输出（如「疑似硬编码密钥」）。报告会上传飞书等协作文档，原样搬运等于二次泄露
...
- **语义分组清单**（可选，仅增量审查时提供）：变更文件的语义关联分组（同模块同功能、组件+hooks/组合式函数+调用点、测试+被测、i18n/配置变体）。提供时，阶段 C 应把同组文件安排在一起评估（同组文件互相作为上下文）；未提供时按原顺序执行。

### 业务背景使用规则

- 业务背景仅用于理解变更意图与业务约束，辅助判断「这是有意为之还是缺陷」；背景**不得替代代码证据**——任何定级仍必须以可见代码证据为准，背景与代码冲突时以代码为准并在必要时标注待确认。
- 背景中用户声明的业务规则（如「该页面仅内网管理后台使用」）可用于调节置信度，但涉及安全定级（P0/P1）时不得仅凭背景免责，需在报告中注明「背景声明与代码证据的关系」。
...
- 必须把批次发现清单写入 `BATCH_RESULT_PATH`，不得写入自造路径；写完后才能把 `BATCH_STATUS_PATH` 写为 `completed`（`result_path` 指向同一文件）。`completed` 状态**不得**携带 `"failure_class"` 字段
...
- 无法完成且零产出时写为 `failed` 并写明错误；状态 JSON 必须包含 `"failure_class"` = 下方「中断归因枚举」之一（判不准写 `unknown`）
...
- **安全设计不变量（与命名、框架和返回类型无关）**：安全维度启用时，完成一个关联审查单元的读取后，必须根据代码真实行为建立内部安全契约：识别受保护动作；识别允许执行该动作所依赖的授权证据；从赋值、分支、异常和组合逻辑归纳全部可达状态；分别追踪每种状态的最终效果；核查主体—资源绑定、外部数据到敏感效果以及敏感数据跨输出边界的传播。不得通过预置组件名、函数名、字段名或框架角色词表代替这一步语义判断。
...
- 安全维度启用且文件属于关联审查单元时，在该单元的安全设计不变量检查完成前，保留受保护动作、授权证据、状态变化和下游效果摘要，不得因单文件内尚不能证成问题而丢弃跨文件语义。
- **安全契约检查点**：安全维度启用时，每个关联审查单元完成后都在检查点文件追加一行结论。没有受保护动作时记录“无受保护动作”；存在时记录“受保护动作 / 授权证据 / 非确定状态 / 默认行为 / 反例是否成立”。该记录只证明语义复核已执行，不直接进入正式问题清单。
...
**背景定向警觉**：业务背景可提示重点风险域（如背景提到「支付收银台组件重构」则对该链路的资金展示与状态竞态检查点提高警觉），但不扩大也不缩小审查范围。

**逐文件覆盖义务（强制）**：
- 结束阶段 C 之前，必须确认对输入清单（批次文件列表 / 审查范围内的源码清单 / 跨文件关联单元）中的**每一个文件**单独完成了至少一遍阅读与评估。
- 文件是关联单元或批次中"较小/次要"的成员（类型定义/`.d.ts`、常量、配置文件、i18n 资源等伴生声明文件）**不是跳过它的理由**；审查了实现文件不等于覆盖了它的接口/配置对应物。
