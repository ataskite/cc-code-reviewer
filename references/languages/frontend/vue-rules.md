...
- **Vue 2 EOL 模板编译器 XSS**：运行时字符串模板（`Vue.compile`、字符串 `template` 选项、动态拼模板）在 EOL 的 Vue 2 版本存在已知模板注入 CVE（CVE-2024-6783 类）；lockfile 锁定 EOL 版本且存在字符串模板用法时必须给升级/迁移建议，不得默认版本安全。
...
*本手册版本：Vue 规则 2.4（Vue2 legacy + Vue3 + B 端组件库补强 + v1.7.1 EOL 模板编译器安全）*
*最后更新：2026-09-20*
