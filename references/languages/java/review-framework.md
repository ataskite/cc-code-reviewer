...
- **多租户隔离**：tenantId / orgId / appId 等隔离条件是否遗漏、可被前端伪造或跨租户访问
- **访问控制**：管理端、内部接口、回调接口是否有来源校验和最小权限控制
- **审计日志**：关键安全动作是否记录操作者、资源、结果、时间和追踪 ID

### 5.3 数据保护与敏感信息

- **敏感信息泄露**：日志、异常、响应、配置、测试数据中是否暴露密码、token、密钥、身份证、手机号等
...
- **认证爆破与用户枚举**：登录 / 注册 / 找回密码 / 短信验证码接口是否做频率限制（IP + 账号维度）与锁定机制；错误信息是否区分"用户不存在"与"密码错误"导致用户枚举；验证码是否可绕过（固定值、可重放、客户端校验）
- **Default Deny / Fail-Closed（默认拒绝）**：接口、资源、工具调用在未显式声明权限时是否默认拒绝访问（而非默认放行）；Filter / Interceptor / 鉴权中间件在异常路径下是否 fail-open（异常时放行）而非 fail-closed（异常时拒绝）；新增接口是否默认继承全局鉴权而非裸露
- **业务逻辑竞态（安全视角）**：余额扣减、优惠券领取、签到积分、抽奖等高价值写操作是否防并发重放（分布式锁 / 幂等键 / 乐观锁 `@Version`）；是否存在 TOCTOU（check-then-use 时间窗口）越权
