...
- **Default Deny / Fail-Closed（默认拒绝）**：接口、资源、工具调用在未显式声明权限时是否默认拒绝访问（而非默认放行）；鉴权判定逻辑在异常/空值/未处理路径下是否 fail-open（放行）而非 fail-closed（拒绝）；新增接口是否默认继承全局鉴权而非裸露。
  - **识别不依赖命名**：鉴权判定逻辑可能出现在任意类中（Filter / Interceptor / 名为 Service 的鉴权链节点 / 策略 / 处理器 / 工具类等），不应仅凭类名是否含 `Filter`/`Security` 决定是否检查。命中以下**代码信号**其一即应就地核查（信号只触发检查、不直接决定级别；级别仍按本手册「安全问题分级边界」与 P0 五项硬门槛判定）：
    - **信号 S1（准入判定函数）**：函数返回 boolean 或布尔语义值（含枚举/Optional），且语义为“判定是否放行/跳过/通过”（常见动词 match / shouldFilter / allow / permit / canAccess / check / verify / validate / skip / handle / preFilter）。此类函数整体是“关卡”，其任何返回 allow/skip/true 的路径都需审视该路径是否对应“已显式验证通过”。
    - **信号 S2（null / 空 / 异常 → 放行）**：在 S1 判定函数内，`== null`、`isEmpty()`、`catch` 块、switch-default、`orElse(true)` 等路径返回了 allow/skip 值，而该路径并不代表“已通过验证”。
    - **信号 S3（语义重载）**：同一字段/返回值在“成功路径”被显式置为 null 或默认值（如鉴权通过后 `setXxx(null)`），使“成功”与“异常未赋值”两种状态不可区分；需跨文件核查该字段所有写入点。
    - **信号 S4（编排无兜底）**：循环/链式遍历多个判定器后，缺少“全部未命中则拒绝”的显式兜底，且框架/容器默认行为是 allow（如网关默认转发、拦截器链默认放行）。
