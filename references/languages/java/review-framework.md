...
- **认证/鉴权 fail-open 即认证绕过**：认证、鉴权或安全开关判定路径上的 fail-open（异常、空值、未处理路径默认放行）按认证绕过类缺陷定级；仓库内代码链完整即视为满足“生产可达 + 证据完整”，运行时触发事件（依赖异常、配置装配失败等常规运维事件）不构成降级理由。网关、鉴权 SDK 等以安全为主业的组件尤其如此：其在依赖降级时刻的表现正是安全设计的核心不变量。
- **身份信任边界缺陷的归类**：客户端可控输入（header、query 参数等）直接成为身份来源的网关/SDK 代码属于认证绕过候选。仓库内代码链完整、最终攻击效果依赖下游部署或配置中心生效值时，归入待确认并显式标注“P0 待验证”，必须给出验证方式，不得静默放入普通 P1。
