...
部分主题会在多个审查维度中出现，例如异常处理、SQL、日志、安全配置、幂等性等。这不是重复定义，而是同一技术点在不同风险语境下的不同表现。

审查时应按维度语义分别判断：
- **正确性**：是否导致业务结果错误
- **资源管理**：是否导致资源泄露或不可恢复状态
- **架构**：是否破坏统一治理和边界设计
- **安全**：是否造成攻击面或敏感信息暴露
- **可观测性**：是否影响定位、追踪和告警

同一代码片段如果同时触发多个风险，可以合并为一条问题，但必须在"所属维度"中标注多个维度。
...
- **SQL / NoSQL 注入**：拼接查询、动态 SQL、`${}`、未绑定参数、Wrapper 拼接风险
- **命令 / SpEL / 模板注入**：`Runtime.exec`、`ProcessBuilder`、SpEL 表达式、模板渲染输入是否可控
- **SSRF**：用户可控 URL 是否可访问内网、云元数据地址或未做协议/域名白名单
- **XXE**：XML 解析是否关闭外部实体、DTD 和外部协议访问
- **路径穿越**：文件路径、下载路径、解压路径是否可被 `../` 或绝对路径绕过
- **输入验证**：外部输入是否有长度、格式、枚举、范围、业务语义校验

### 5.2 认证、授权与租户隔离

- **认证与授权**：接口是否缺少认证、权限注解或统一鉴权拦截
- **对象级越权**：是否只校验登录态，而未校验资源归属、组织、角色或数据权限
- **多租户隔离**：任何标识租户/组织/商户/部门归属的隔离条件（不限于 tenantId/orgId/appId 等固定命名）是否遗漏、可被前端伪造或跨租户访问
- **授权面差分反例**：Security 模式完成逐文件阅读后，按统一 Security 框架再次核对同角色异对象、低权限高功能、跨租户、对象属性、批量/嵌套资源和替代执行路径；Spring Security/Shiro 注解、helper 名称和不可预测 ID 只能作为线索，不能替代实际决策与查询约束证据
- **访问控制**：管理端、内部接口、回调接口是否有来源校验和最小权限控制
- **审计日志**：关键安全动作是否记录操作者、资源、结果、时间和追踪 ID

### 5.3 数据保护与敏感信息

- **敏感信息泄露**：日志、异常、响应、配置、测试数据中是否暴露敏感字段——按数据分类（凭据、PII、财务、健康等）识别，**不依赖固定字段名清单**（密码/token/密钥/身份证/手机号仅为示例，apiKey/accessKey/credential/privateKey 等同样适用）；**间接泄露**：实体 `@Data`/`toString` 含敏感字段被 logger 间接打印、DTO 序列化暴露过多字段、异常堆栈携带敏感对象
- **数据暴露**：DTO / VO 是否返回过多字段，是否存在批量导出、分页绕过或未脱敏响应
- **敏感字段分层保护**：手机号、身份证、邮箱、银行卡等个人信息（PII，Personally Identifiable Information；国内合规语境按个人信息/敏感个人信息分类）是否按场景实施分层保护：存储侧按数据分类和业务必要性选择掩码、不可逆哈希、字段级加密或受控明文，仍需还原的字段不得误用哈希；传输侧仅返回必要字段且不向无权角色返回完整值；日志侧通过日志框架或拦截器统一脱敏，禁止 `toString` 泄露；第三方外传前剥离非必要字段。DTO 序列化优先使用 `@JsonSerialize` 脱敏注解或统一序列化器，而非逐处手写。不得只因数据库存在受控明文就直接判定为漏洞，必须结合数据分类、访问控制、加密边界和适用合规要求形成证据链
- **加密与密钥管理**：密码是否强哈希，敏感数据是否加密存储，密钥是否硬编码或进入仓库
- **错误信息安全**：错误响应是否暴露堆栈、SQL、路径、内部类名或权限判断细节

### 5.4 文件、序列化与外部资源

- **文件上传**：类型、大小、后缀、MIME、内容嗅探、存储路径和访问权限是否受控
- **文件下载 / 预览**：是否存在任意文件读取、越权下载或响应头缺失
- **反序列化**：Java 原生序列化、JSON 多态反序列化、白名单和类型限制是否安全
- **压缩包处理**：是否存在 Zip Slip、压缩炸弹和解压后文件数量/大小失控风险

### 5.5 会话、JWT 与安全配置

- **JWT 安全**：签名算法、密钥长度、过期时间、刷新机制、黑名单/撤销机制是否合理
- **会话安全**：Cookie `HttpOnly` / `Secure` / `SameSite`、会话固定、防重放是否处理
- **CSRF 风险**：浏览器态接口是否需要 CSRF 防护或 SameSite 策略
- **安全配置**：CORS、Actuator、Swagger、错误页、debug 配置、默认账号、管理端暴露是否安全

### 5.6 依赖与供应链安全

- **依赖安全**：依赖版本是否存在已知漏洞，是否建议接入 SCA / SBOM / 漏洞扫描
- **构建与插件安全**：Maven / Gradle 插件、私服、仓库源、快照依赖是否可信
- **许可证与合规**：企业合规场景下是否存在明显许可证风险或未声明第三方组件
...
> **面向公网可达（外网应用）的接口与页面**。当项目存在 Web 入口（Spring MVC Controller、网关路由、对外开放 API）时启用本章节，覆盖外网高频被攻击路径。内网-only 服务可适当降低本章节权重，但不得跳过认证爆破与默认拒绝检查。

- **XSS（跨站脚本，服务端拼接）**：Controller / Service 直接拼接 HTML 字符串返回用户输入是否做 HTML 转义；模板引擎手动关闭转义是否安全（Thymeleaf `th:utext`、JSP `<%=...%>`、FreeMarker `?html` 缺失或 `escape=false`）；富文本输入是否做白名单过滤而非黑名单
- **CSRF（跨站请求伪造）**：浏览器态（Cookie/Session）写操作接口是否校验 CSRF token 或依赖 SameSite Cookie；状态变更接口（下单、改密、转账）是否可被跨站构造请求触发；前后端分离 + JWT 场景需确认不依赖 Cookie 即不涉 CSRF 的前提成立
- **开放重定向**：重定向 URL（`redirect:`、`Location` 头、`callback` 参数）是否用户可控且未做白名单校验；是否可被构造为钓鱼跳板（`https://evil.com`）
- **CORS 配置错误**：credentialed request 与 `Access-Control-Allow-Origin: *` 的组合会被浏览器 CORS 校验拒绝，通常属于配置或功能错误，不得误报为“任意站点可携带凭据读取响应”；重点检查服务端是否原样反射任意 `Origin`，或使用过宽 Origin 白名单并同时允许 credentials，这才可能让攻击者站点读取携带凭据的跨域响应
- **HTTP 安全响应头**：是否缺失 `Strict-Transport-Security`（HTTPS 强制）、`X-Frame-Options` / `frame-ancestors`（Clickjacking）、`X-Content-Type-Options: nosniff`（MIME 嗅探）、`Content-Security-Policy`（XSS 纵深防御）；生产环境是否仍暴露 `Server` / `X-Powered-By` 指纹头
- **认证爆破与用户枚举**：涉及账号凭证签发、验证码、密码重置的写接口（按功能语义识别，不限于"登录/注册/找回密码"等 URL 关键词）是否做频率限制（IP + 账号维度）与锁定机制；错误信息是否区分"用户不存在"与"密码错误"导致用户枚举；验证码是否可绕过（固定值、可重放、客户端校验）
- **Default Deny / Fail-Closed（默认拒绝）**：接口、资源、工具调用在未显式声明权限时是否默认拒绝访问（而非默认放行）；鉴权判定逻辑在异常/空值/未处理路径下是否 fail-open（放行）而非 fail-closed（拒绝）；新增接口是否默认继承全局鉴权而非裸露。
  - **按安全契约识别，不按代码词汇识别**：先从代码行为推断受保护动作以及允许执行该动作所需的明确授权证据，再还原证据的生产、传播、解释和最终效果。类名、方法名、字段名、返回类型和框架角色都不能单独决定是否检查。
    - **不变量 S1（明确授权）**：只有可证明授权条件已经成立的状态，才能到达受保护动作。授权证据必须来自可信来源，并且在传播过程中没有丢失主体、资源、范围或时效语义。
    - **不变量 S2（状态完备）**：根据实际赋值、分支、异常和组合逻辑归纳授权证据的全部可达状态；缺失、未知、异常、未初始化、未匹配或无法解释的状态必须与明确成功区分。
    - **不变量 S3（逐状态结果）**：分别追踪每种状态最终进入拒绝、终止、降权还是受保护动作。任何尚未获得明确授权证据却到达受保护动作的路径，都是 Default Deny / Fail-Closed 候选。
    - **不变量 S4（编排默认行为）**：对多段处理、策略选择、责任链或框架回调，核查所有参与者均未作出明确允许或拒绝决定时系统实际执行什么；不能把“无人处理”“没有结果”或异常退出默认为允许。
  - **反例驱动取证**：模型应主动尝试构造“未明确授权状态 → 受保护动作”的可达路径。检索调用方、写入点、实现和下游效果时，只使用当前代码中已经识别出的现场符号，不使用预置命名词表。反例成立后仍须按本手册的安全分级边界与 P0 五项硬门槛定级。
- **业务逻辑竞态（安全视角）**：高价值写操作（资产变动、额度消耗、稀缺资源发放等——按价值语义识别，不限于余额/优惠券/签到/抽奖等场景）是否防并发重放（分布式锁 / 幂等键 / 乐观锁 `@Version`）；是否存在 TOCTOU（check-then-use 时间窗口）越权
...
- 关键路径性能风险且已证实会显著影响稳定性：P1。示例：关键同步链路外部调用无超时、无界并发、连接池耗尽、明显 OOM/线程泄漏、缓存击穿/雪崩/一致性问题影响核心能力。
- 普通性能问题或局部性能风险：P2。示例：N+1 风险但缺少执行计划证据、循环嵌套、对象创建过多、分页不合理、重试策略不当。
- 泛优化建议且缺少明确风险链路：P3。示例：建议加缓存、建议补监控、建议优化算法或拆分任务但没有明确生产影响证据。
- 怀疑很严重但缺少生产路径、调用频率、运行配置、表结构、索引或执行计划证据：待确认。不得仅凭“可能慢”“可能全表扫描”“可能打爆资源”的印象升级到 P1/P0。

### 安全问题分级边界

- 安全问题必须先逐项核验 P0 五项硬门槛；公网/内网、匿名/已认证以及其他利用前置条件只作为生产可达性、影响范围和有效防护的证据，不得单独决定级别上限。
- 生产可达且证据完整的 RCE、命令注入、可窃取高权限云凭据的 SSRF、认证绕过、严重越权、SQL 注入或存储型 XSS 等漏洞，只有同时满足事故级影响、缺少有效防护和必须阻断发布时才标为 P0。网络位置或登录前置条件不自动降级；例如已认证 RCE 或内网管理面命令注入仍可能满足 P0 五项硬门槛。
- 证据成立但影响未达到事故级，或已有能够显著限制利用与影响的有效缓解机制：P1。示例：只影响非核心资源、低价值写操作的 CSRF、缺少安全响应头但不构成独立事故。仅“位于内网”或“需要登录”本身不足以判定为 P1。
- 泛化安全加固建议、缺少可达路径证据或仅依赖代码形态推测：P2/P3。示例：建议升级依赖版本但无明确 CVE、建议补全审计日志、建议细化权限注解但没有越权证据。
- 怀疑是高危但缺少生产可达性、调用链或运行配置证据：待确认。不得仅凭“可能未校验”“可能可绕过”“默认不安全”的印象升级到 P1/P0，必须证实攻击路径成立。

### 高危安全问题与安全设计不变量定级规则

- **高危安全问题必为 P0**：凡是同时满足生产可达、证据完整且高置信、事故级安全影响、缺少有效防护的安全问题，必须标为 P0 并阻断发布；不得以触发概率低、触发条件非攻击者可控、缺陷位于异常路径等理由降级到 P1。触发条件与触发概率只影响事故发生频率的评估，不改变证据完整性与影响等级；反向降级必须证明触发路径生产不可达，而不是断言“低概率”。
- **认证/鉴权 fail-open 即认证绕过候选**：认证、鉴权或安全开关判定路径上的 fail-open（异常、空值、未处理路径默认放行）按认证绕过类缺陷定级；代码链闭合只能证明静态触发路径和缺少拒绝分支，只有入口注册、装配以及部署/运行配置也由仓库证据闭合时才满足生产可达。否则归入待确认并显式标注“P0 待验证”；依赖异常、配置装配失败等常规运维事件不构成低概率降级理由。网关、鉴权 SDK 等以安全为主业的组件尤其如此：其在依赖降级时刻的表现正是安全设计的核心不变量。
- **身份信任边界缺陷的归类**：客户端可控输入（header、query 参数等）直接成为身份来源的网关/SDK 代码属于认证绕过候选；最终攻击效果依赖下游部署或配置中心生效值时，归入待确认并显式标注“P0 待验证”，必须给出验证方式，不得静默放入普通 P1。
...
- **security**（安全专项）：聚焦安全核心及与安全强相关的交叉维度。security 模式必须同时读取并执行 `references/security/enterprise-security-framework.md`（跨语言权威依据：安全语义、取证、证据等级与分级规则），本文件与 Java 规则只负责维度与实现映射，不得降低其安全要求。
