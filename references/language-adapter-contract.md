...
SOURCE_SCOPE:formal|./*.js
SOURCE_SCOPE:context|**/*.test.tsx
SOURCE_SCOPE:excluded|node_modules/**
```

`SERVER_ROOT:formal|{相对路径}`（可重复）声明 BFF server-root 层的包根（前端专属，v1.7.0）：该包根及一级目录内的服务端 `.js/.mjs/.cjs` 属于正式范围，其准确边界以不可变 source manifest 为准。无 server 层的适配器不输出该行。
...
- `COMPONENT` / `TECH_STACK` / `SOURCE_SCOPE` 可重复出现。
  ...
  - 前端 source manifest 收敛必须通过 `scripts/languages/frontend/filter-source-manifest.sh`：`src/components` 或 `components` 匹配所有支持技术栈的 package-local `*/src/components/`，`apps/web/src/components` 只匹配对应 package；BFF server 层目录（`SERVER_ROOT:formal` 包根一级目录，如 `controllers`）按包根前缀命中；绝对路径和 `..` 路径穿越必须拒绝。
