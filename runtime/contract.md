...
- 每个状态等待用户响应后才能进入下一状态。
- `current_scope_sizing` 必须按已确认的 `review_scope` 重算文件数和行数；只有 `estimated_tokens > 1000000` 时才允许进入 `optional_batch_strategy`。
- `estimated_tokens <= 1000000` 的 Maven 多模块存量审查跳过 `optional_batch_strategy`，保持 `single-agent`；全量审查不得显示“所选模块”文案。
