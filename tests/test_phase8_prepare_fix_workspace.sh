...
CURRENT_OUTPUT="$(bash "$ROOT_DIR/scripts/core/prepare-fix-workspace.sh" "$TMP_DIR" current "")"
...
BRANCH_OUTPUT="$(bash "$ROOT_DIR/scripts/core/prepare-fix-workspace.sh" "$TMP_DIR" branch "fix/review-findings")"
...
WORKTREE_OUTPUT="$(bash "$ROOT_DIR/scripts/core/prepare-fix-workspace.sh" "$TMP_DIR" worktree "fix/worktree-findings")"
...
if bash "$ROOT_DIR/scripts/core/prepare-fix-workspace.sh" "$TMP_DIR" worktree "fix/worktree-findings" >"$WORKTREE_EXISTS_OUTPUT" 2>&1; then
...
EXISTING_WORKTREE_OUTPUT="$(bash "$ROOT_DIR/scripts/core/prepare-fix-workspace.sh" "$TMP_DIR" worktree "fix/existing-worktree")"
...
if bash "$ROOT_DIR/scripts/core/prepare-fix-workspace.sh" "$TMP_DIR" branch "fix/dirty" >"$DIRTY_BRANCH_OUTPUT" 2>&1; then
...
if bash "$ROOT_DIR/scripts/core/prepare-fix-workspace.sh" "$TMP_DIR" worktree "fix/dirty-worktree" >"$DIRTY_WORKTREE_OUTPUT" 2>&1; then
...
if bash "$ROOT_DIR/scripts/core/prepare-fix-workspace.sh" "$TMP_DIR" branch "bad branch name" >"$INVALID_OUTPUT" 2>&1; then
...
if bash "$ROOT_DIR/scripts/core/prepare-fix-workspace.sh" "$TMP_DIR" branch "" >"$EMPTY_BRANCH_OUTPUT" 2>&1; then
...
if bash "$ROOT_DIR/scripts/core/prepare-fix-workspace.sh" "$TMP_DIR/missing" current "" >"$MISSING_PROJECT_OUTPUT" 2>&1; then
...
if bash "$ROOT_DIR/scripts/core/prepare-fix-workspace.sh" "$NON_GIT_DIR" current "" >"$NON_GIT_OUTPUT" 2>&1; then
...
if bash "$ROOT_DIR/scripts/core/prepare-fix-workspace.sh" "$TMP_DIR" unknown "fix/unknown" >"$UNKNOWN_MODE_OUTPUT" 2>&1; then
...
if bash "$ROOT_DIR/scripts/core/prepare-fix-workspace.sh" "$EXISTING_MUTATION_DIR" worktree "fix/existing" >"$EXISTING_MUTATION_OUTPUT" 2>&1; then
...
if bash "$ROOT_DIR/scripts/core/prepare-fix-workspace.sh" "$DIRTY_MUTATION_DIR" worktree "fix/dirty" >"$DIRTY_MUTATION_OUTPUT" 2>&1; then
