...
echo "$OUTPUT" | grep -Fq "SUPERPOWER_MISSING=using-git-worktrees,finishing-a-development-branch"
...
mkdir -p "$FIRST_ROOT/brainstorming" "$FIRST_ROOT/test-driven-development" "$SECOND_ROOT/using-git-worktrees" "$SECOND_ROOT/verification-before-completion" "$SECOND_ROOT/finishing-a-development-branch"
...
mkdir -p "$GLOB_ROOT/brainstorming" "$GLOB_ROOT/using-git-worktrees" "$GLOB_ROOT/test-driven-development" "$GLOB_ROOT/verification-before-completion" "$GLOB_ROOT/finishing-a-development-branch" "$GLOB_EXPANSION_ROOT"
