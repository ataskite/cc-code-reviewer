...
#   2. 核心用例点估计：
#      - 前端 deep 209 文件 / 15,882 行（用户原始误报项目）→ 6-10 分钟区间
#      - Java fast 小项目 42 文件 / 3,850 行 → 下限 1 分钟
#      - Java deep 186 文件 / 28,500 行 → 8-13 分钟区间
#   3. CONTEXT_SCALE 缩放：1M 窗口（scale=5）分钟数约为 scale=1 的 1/5
...
# ── 2. 核心用例点估计 ──
# 用户原始误报项目：209 文件 / 15,882 行 / deep → cost=21107 → 21107×15/52000=6.09 → 7 分钟
# （修正前固定表误报为 30-50 分钟，虚高约 5 倍）
FE_MIN="$(minutes_of deep 15882 209)"
[ "$FE_MIN" = "7" ] || { echo "FAIL: 前端 deep 应为 7 分钟，实际 $FE_MIN" >&2; exit 1; }
FE_RANGE="$(range_of deep 15882 209)"
[ "$FE_RANGE" = "6-10 分钟" ] || { echo "FAIL: 前端 deep 区间应为 6-10 分钟，实际 $FE_RANGE" >&2; exit 1; }
...
# Java deep 186 文件 / 28,500 行 → cost=33150 → 33150×15/52000=9.56 → 10 分钟
JA_DEEP_MIN="$(minutes_of deep 28500 186)"
[ "$JA_DEEP_MIN" = "10" ] || { echo "FAIL: Java deep 应为 10 分钟，实际 $JA_DEEP_MIN" >&2; exit 1; }
JA_DEEP_RANGE="$(range_of deep 28500 186)"
[ "$JA_DEEP_RANGE" = "8-13 分钟" ] || { echo "FAIL: Java deep 区间应为 8-13 分钟，实际 $JA_DEEP_RANGE" >&2; exit 1; }

# ── 3. CONTEXT_SCALE 缩放（1M 窗口 scale=5 → 分钟数约为 scale=1 的 1/5） ──
SCALE1_MIN="$(minutes_of deep 15882 209 1)"
SCALE5_MIN="$(minutes_of deep 15882 209 5)"
[ "$SCALE1_MIN" = "7" ] || { echo "FAIL: scale=1 应为 7 分钟" >&2; exit 1; }
[ "$SCALE5_MIN" = "2" ] || { echo "FAIL: scale=5 应为 2 分钟（7/5 向上取整），实际 $SCALE5_MIN" >&2; exit 1; }

# ── 4. 各模式系数（同一规模下 fast < security < standard < deep） ──
M_FAST="$(minutes_of fast 52000 0)"
M_SEC="$(minutes_of security 52000 0)"
M_STD="$(minutes_of standard 52000 0)"
M_DEEP="$(minutes_of deep 52000 0)"
# cost=52000（满批次）时：fast=4, security=10, standard=8, deep=15
