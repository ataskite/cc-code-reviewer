...
- **交互式 Scan**：预扫描后逐步确认分支、模式、输出、入口、范围和执行计划。
- **可恢复分批扫描**：Maven 多模块项目按模块或语义单元规划批次，其他大项目按文件 token 预算规划批次，支持跨会话续跑。
...
1. 分支选择：仅多分支 Git 仓库时询问，且在审查配置前完成
2. 审查模式：`fast`、`standard`、`deep`、`security`
3. 审查结果处理方式：默认本地 Markdown 报告；仅 lark-cli 可用时提供飞书选项
4. 审查入口：增量审查、全量审查或指定模块
5. 审查范围：增量审查选择最近 N 次提交；指定模块审查选择或输入模块路径
6. 存量审查方式：Maven 多模块存量审查选择“按所选模块依次启动”或“AI 智能规划分批”
7. 本轮执行批次：仅分批审查时展示用户可见的 Markdown 批次表、可执行批次和推荐计划
8. 并发数选择：仅本轮执行多批时询问，选项不会超过实际执行批次数
9. 最终执行计划确认
...
存量审查规模较大时，Scan 会先生成可恢复的批次任务，再让用户选择本轮执行哪些批次。批次状态保存在项目目录：

```text
.cc-code-reviewer/runs/{YYYYMMDD-HHMMSS}-{branch_slug}-{REVIEW_MODE}
```

`RUN_DIR` 名称只包含时间、分支和审查模式；审查范围、分批策略、总行数、批次数和选中模块等信息都记录在 `plan.json` 中。后续展示状态、续跑和合并报告都以 `plan.json` 和 `batches/*.json` 为准。

预扫描、分批计划和批次 agent 的正式扫描只统计 `src/main/java` 生产源码；`src/test/java` 等测试源码可作为测试质量上下文，但不计入审查规模、批次文件清单或正式问题位置。

Maven 多模块存量审查使用 Maven planner：

- 全量审查和指定模块审查都支持进入 Maven 分批模式。
- 用户可以选择“按所选模块依次启动”：每个所选模块形成一个批次，适合按业务域推进。
- 用户可以选择“AI 智能规划分批”：按 Maven 模块、包目录、代码行数和 review cost 自动拆分，适合超大模块或希望批次更均衡时使用。
- 即使只选择一个 Maven 模块，只要选择了 Maven 多模块存量分批策略，也会使用 Maven planner，并把 `REVIEW_SCOPE` 限定在所选模块内。
- jdtls-lsp 可用时，批次 agent 必须使用 definition、references、implementations、call hierarchy 辅助理解跨目录调用链；不可用时回退 Maven 静态依赖和文本检索。

非 Maven 多模块的大项目在触发分批时使用文件级 planner：

- 覆盖 Maven 单模块、Gradle 或未知 Java 项目。
- 按文件风险优先级、行数和 token 预算生成 `file-token-batching` 批次。
- 规划后会展示“简要分批计划”，再进入并发选择。

批次执行时，插件会直接在对话中展示 Markdown 状态表，表头固定为：
...
可执行批次只包含待执行和失败待重试批次；已完成批次会保留状态但不重复调度。推荐执行计划会根据当前可执行批次数动态生成，批次很少时不会出现不可能的“执行 5 批”或“执行 10 批”选项。并发数最多 3 路，但始终不超过本轮实际执行批次数；只执行 1 批时自动串行。

子 agent 只在本批 `scan_roots` 内输出正式问题，跨批风险记录为“跨批依赖待复核”。合并脚本以本轮 `RUN_BATCH_IDS` 为门禁：先检查本轮主任务批次状态，`pending/running` 会等待到完成或超时，`failed`、结果缺失或等待超时会生成 `[合并阻塞]` 报告并提示遗留批次；本轮批次均完成后才合并已完成结果。若还有未纳入本轮的批次，报告标记为 `[阶段性]` 并在批次状态总览中列出遗留；全部批次均完成后生成完整报告。
