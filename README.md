...
- macOS / Linux
- Bash 3.0+、`git`（建议 ≥ 2.41，更低版本在分支预检中警告但不阻断）、系统自带 `perl`
...
### 增量对照与输出增强（v1.6.6 / v1.6.8 / v1.6.9）
...
- 敏感路径强制排除（v1.6.9）：`.env` / `.env.*`（example/sample/template 模板仅在未命中其他敏感路径规则时豁免）、`.ssh/**`、id_* 私钥、`.netrc`/`.npmrc`/`.pypirc`/`.dockercfg` 等凭据样路径在冻结输入层即被强制排除（`exclude_reason=secret-path`，优先级最高、清单命中不可恢复、rename 旧路径同判），密钥文件永不进入正式审查范围
- 证据脱敏（v1.6.9）：三个审查 agent 的证据与建议中不得原样复制密钥/口令/令牌值——引用键名与位置、值以 `***` 掩码，「疑似硬编码密钥」类发现照常输出不降级；报告会进入飞书等协作文档，脱敏防止二次泄露
...
Maven 大仓规划（v1.6.9）只遍历一次 `src/main/java` 并在单进程内统计行数，随后把目录级 LOC / 文件数一次性预聚合到对象格式无关的磁盘索引；模块和 package 查询不再重复扫描 manifest 或逐文件启动 `wc`。POM 中可规范化的 `./`、`../` 模块片段统一转换为仓库相对物理路径；SHA-1 / SHA-256 仓库的根提交增量范围均动态使用对应对象格式的空 tree OID。
